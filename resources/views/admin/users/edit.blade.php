        ...
        <div class="w-full bg-gradient-to-r from-slate-800 to-slate-900 border-b border-slate-700 sticky top-0 z-10 overflow-hidden">
            <div class="px-3 sm:px-6 lg:px-8 py-4">
                <h1 class="text-2xl sm:text-3xl font-bold text-white flex items-center gap-2 sm:gap-3">
                    ...
                    <span class="truncate">Edit {{ $user->name }}</span>
                </h1>
                <p class="text-slate-400 mt-1 text-sm">Update user information and permissions</p>
            </div>
        </div>
            ...
            <div class="w-full max-w-2xl mx-auto">
                <div class="bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="name" class="block text-slate-200 font-semibold mb-2 text-sm">Name</label>
                        ...
                        <div class="mb-4">
                            <label for="email" class="block text-slate-200 font-semibold mb-2 text-sm">Email</label>
                        ...
                        <div class="mb-6">
                            <label for="is_admin" class="flex items-center">
                                ...
                                <span class="text-slate-200 font-semibold text-sm">Is Admin</span>
                            </label>
                        </div>

                        <div class="flex gap-2 sm:gap-4">
                            <button type="submit" class="flex-1 px-3 sm:px-4 py-1 sm:py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-xs sm:text-sm font-semibold transition">✅ Save</button>
                            <a href="{{ route('admin.users.index') }}" class="flex-1 px-3 sm:px-4 py-1 sm:py-2 bg-slate-600 text-white rounded hover:bg-slate-700 text-center text-xs sm:text-sm font-semibold transition">← Cancel</a>
                        </div>
                    </form>
                    </div>
                </div>
                ...
                <div class="mt-6 bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-4 sm:p-6">
                        <div class="mb-4 p-3 sm:p-4 bg-red-900/20 border border-red-600/50 rounded-lg">
                            <h3 class="text-red-400 font-semibold mb-2 text-sm">🔐 Reset Password</h3>
                            <p class="text-red-300 text-xs mb-3">Generate a new temporary password and send it to the user via email. The user will be required to change it on their next login.</p>
                        </div>
                            ...
                            <div class="bg-slate-800 rounded-lg shadow-xl border border-slate-700 max-w-sm w-full">
                                <div class="p-6">
                                    ...
                                    <p class="text-slate-400 text-sm mb-4">
                                        You're about to send a new password to <span class="font-semibold text-white">{{ $user->name }}</span> ({{ $user->email }}).
                                    </p>

                                    <!-- Info Box -->
                                    <div class="bg-amber-900/30 border border-amber-600/50 rounded-lg p-3 mb-6">
                                        <p class="text-amber-300 text-xs">
                                            ✉️ A temporary password will be emailed to the user. They must change it on their next login.
                                        </p>
                                    </div>
